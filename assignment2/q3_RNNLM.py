...
class RNNLM_Model(LanguageModel):
    ...
    def add_embedding(self):
        ...
        with tf.device('/cpu:0'):
            ...
            with tf.variable_scope("embedding"):
                L = tf.get_variable("L", (len(self.vocab), self.config.embed_size), initializer=xavier_weight_init())
                input_wvs = tf.nn.embedding_lookup(L, self.input_placeholder)
                input_wvs = tf.split(input_wvs, self.config.num_steps, 1)
                inputs = []
    ...
    def add_projection(self, rnn_outputs):
        ...
        with tf.variable_scope("projection"):
            U = tf.get_variable("U", (self.config.hidden_size, len(self.vocab)), initializer=xavier_weight_init())
    ...
    def add_model(self, inputs):
        ...
        with tf.variable_scope("hidden"):
            H = tf.get_variable("U", (self.config.hidden_size, self.config.hidden_size),
        ...
        for i in range(self.config.num_steps):
            rnn_outputs.append(tf.nn.sigmoid(tf.matmul(inputs[i], I) + tf.matmul(rnn_outputs[i], H) + b1))
        del rnn_outputs[0]
