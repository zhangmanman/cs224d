...
class SoftmaxModel(Model):
    ...
    def load_data(self):
        ...
        np.random.seed(1234)
        self.input_data = np.random.rand(   ##1024*100
            self.config.n_samples, self.config.n_features)
        self.input_labels = np.ones((self.config.n_samples,), dtype=np.int32) ##1024*1
    ...
    def add_model(self, input_data):
        ...
        with tf.name_scope('weights') as scope:
            W = tf.Variable(tf.zeros((self.config.n_features, self.config.n_classes)), dtype=np.float32, name='weights')
            b = tf.Variable(tf.zeros((self.config.batch_size, self.config.n_classes)), dtype=np.float32, name='bias')
