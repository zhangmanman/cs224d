...
def softmax(x):
    ...
    xNew = tf.subtract(x, xMax)
    xExp = tf.exp(xNew)
    xExpSum = tf.reduce_sum(xExp, axis=1)
    xExpSum = tf.expand_dims(xExpSum, 1)
    out = tf.divide(xExp, xExpSum)
...
def cross_entropy_loss(y, yhat):
    ...
    ### YOUR CODE HERE
    yhatNew = tf.log(yhat)
    outPre = tf.multiply(tf.to_float(y), yhatNew)
    # with tf.Session():
    #     xE10 = outPre.eval()
    out = -tf.reduce_sum(outPre)
    # with tf.Session():
    #     xE11 = out.eval()
    # raise NotImplementedError
    ### END YOUR CODE
