...
class NERModel(LanguageModel):
    ...
    def create_feed_dict(self, input_batch, dropout, label_batch=None):
        ...
        if label_batch is not None:
            ...
            return {self.input_placeholder: input_batch, self.dropout_placeholder: dropout}
            # raise NotImplementedError
            ### END YOUR CODE
            # return feed_dict
    ...
    def add_embedding(self):
        ...
        with tf.device('/cpu:0'):
            ### YOUR CODE HERE
            temp = tf.nn.embedding_lookup(self.wv, self.input_placeholder)
            window = tf.reshape(temp, (-1, self.config.window_size * self.config.embed_size))
            # raise NotImplementedError
            ### END YOUR CODE
            return window
    ...
    def add_model(self, window):
        ...
        with tf.variable_scope("Layer"):
            self.W = tf.Variable(xavier_initializer((self.config.window_size * self.config.embed_size, self.config.hidden_size)))
            self.b1 = tf.Variable(xavier_initializer((self.config.hidden_size,)))
        with tf.variable_scope("Softmax"):
            self.U = tf.Variable(
                xavier_initializer((self.config.hidden_size, self.config.label_size)))
            self.b2 = tf.Variable(xavier_initializer((self.config.label_size,)))
        ...
        h = tf.nn.tanh(tf.matmul(tf.to_float(window), self.W) + self.b1)
        output = tf.matmul(tf.nn.dropout(h, self.dropout_placeholder), self.U) + self.b2
        # output = tf.nn.softmax(tf.matmul(tf.nn.dropout(h, self.dropout_placeholder), self.U) + self.b2)
