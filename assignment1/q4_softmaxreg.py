...
def getSentenceFeature(tokens, wordVectors, sentence):
    ...
    for word in sentence :
        index = tokens[word]
        wordVector = wordVectors[index]
        sentVector += wordVector / sentenceLen
...
def softmaxRegression(features, labels, weights, regularization = 0.0, nopredictions = False):
    ...
    # temp = np.zeros((N, weights.shape[1]))
    temp = np.copy(prob)
    # temp[range(N), labels] = prob[range(N), labels]
    temp[range(N), labels] -= 1
    grad = (np.dot(features.T, temp)) / N + regularization * weights
